...
<nav class='navbar navbar-expand-lg navbar-light shadow'>
	...
	$cart = mysqli_query($conShop, "SELECT * FROM shoppingCart WHERE ProductId=$ProductId AND clientId =$_SESSION[loggedInUserId]");
    $product = mysqli_query($conShop, "SELECT * FROM products WHERE ProductId=$ProductId");
	
    
    if($rowCart = mysqli_fetch_array($cart)){
	...
	mysqli_query($conShop, "insert into orders (orderId, ProductId, Price, Quantity, DateOfOrder,clientId)
    values ('$orderId', $ProductId, $totalPrice, $quantity, '$todaydate',$_SESSION[loggedInUserId])");
	
	mysqli_query($conShop, "DELETE FROM shoppingCart WHERE ProductId=$ProductId AND clientId =$_SESSION[loggedInUserId]");

	
echo"
    <h1><a href='Orders.php'>ORDERS</a></h1>
    <h2>Items Added: .$rowProduct[Title].</h2>
	<h2>Order Number: .$orderId. </h2>
";
}
else{

    echo "<h1>YOU DID NOT BUY ANYTHING</h1>";
}
	?>
